...
def main():
    ...
    train(
        train_loader, model_with_lora, device, optimizer_with_lora, criterion, epochs=1
    )
    ...
    digit_to_finetune = 9
    indices = [
        i for i, y in enumerate(full_train_dataset.targets) if y == digit_to_finetune
    ]
    ...
    print(
        f"Trainable parameters without LoRA: {count_trainable_params(model_without_lora)}"
    )
